...
def edit(id):
    ...
    return render_template('Addform.html',
                           ...
                           class_take=class_take, sch=sch, stat=['участник', 'призёр', 'победитель'], olim=olim, teach=teach, year=year)
...
def profile():
    ...
    if request.method == "POST":
        ...
        if res:
            ...
            res.patronymic = request.form.get('patronymic')
            res.school_id = db_sess.query(School.id).filter(request.form.get('schools') == School.name).first()[0]
            p = db_sess.query(User).filter(User.email == request.form.get('username')).all()
            if len(p) == 0:
